...
# What it does:
Automatically refreshes on NTA, acquires on Acquire if available, and can submit based on timer position, without triggering a third-party extension ping. It does not currently perform any additional mouse movements to keep the extension awake, so keep that in mind. The included images are based on a maximized Chrome window at a screen resolution of 1920x1080; if you use a different resolution, you will need to edit your own photos to match. The auto-submit definitely needs tuning, it seems to work great on longer (9+ minutes) tasks, not so well yet on shorter ones. Probably needs additional reference photos.
